...
Resolve Free/Studio 都能从 Workspace -> Scripts 运行 Python 脚本，
因此这里用本地 JSON-RPC 把 Resolve Python API 暴露给同目录的 Electron 界面。
"""
...
def find_electron(script_dir):
    ...
    if config.is_file():
        try:
            paths.insert(0, Path(config.read_text(encoding="utf-8").strip()))
        except OSError:
            pass
...
def load_resolve(electron_path):
    ...
    os.environ.setdefault("RESOLVE_SCRIPT_LIB", str(resolve_root / "fusionscript.dll"))
    api_root = Path(os.environ.get("RESOLVE_SCRIPT_API", ""))
...
def build_am_inputs(line, fps):
    ...
    return {"Lyrics": "|".join(segments), "Timings": "|".join(timings), "Duration": (line["endMs"] - line["startMs"]) / 1000, "Offset": 0, "FPS": fps}
...
def main():
    ...
    app_dir = script_dir / "AMLL-Lyrics-App"
...
if __name__ == "__main__":
    try:
        ...
        print(f"AMLL 歌词助手启动失败：{error}", file=sys.stderr)
