...
- **Controls**：歌词、分词时间、自动时长、偏移、帧率、可直接编辑的起始／结束字符、分段结果、输入检查；宏自身提供分段与恢复默认按钮。
- **Style**：字体、字重、字号、位置、字距、行距、颜色和总透明度。
- **Motion**：亮暗前沿宽度（em）、未唱／已唱透明度、上浮高度（em）、最短上浮时长、强调门槛／倍率、光晕倍率、句尾加强、背景人声模式。
- 移除未接线的模糊滑杆、重复的亮度／透明度控制和隐藏的字符标记状态。
- **恢复默认效果**恢复字体、位置、颜色和动效；**不改变 Lyrics、Timings、Duration、Offset、FPS 或分段选择**，不会破坏已完成的对拍。
...
4. 不想手写分隔符时，直接填写“分段起始字符”和“分段结束字符”，再点击“按字符范围添加分段”。字符从 **1** 开始，结束字符包含在选区内，`|` 不计数。
5. 例如“我想要留住”选择 2–3，得到 `我|想要|留住`。已有边界保留；已有时间按原段比例拆分，原有空拍保留。反向、越界或非法时间不会修改歌词。
6. 使用“恢复默认效果（保留歌词和时间）”回到推荐外观。
