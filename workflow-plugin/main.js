...
async function execute(action, args = {}) {
  if (action === 'status') {
    ...
    try { timeline = (await host.context()).info; } catch (error) { hostError = error.message.includes('Cannot find module') ? '未安装达芬奇原生桥接文件，请运行安装脚本后从 Studio 打开。' : error.message; }
  ...
  if (['export', 'render'].includes(action)) {
    ...
    if (action === 'render' && renderer.available !== true) throw new Error(renderer.reason);
    let job;
