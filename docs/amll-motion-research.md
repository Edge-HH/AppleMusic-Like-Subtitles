...
| 普通上浮 | 0 → -0.05em，ease-out，至少 1000ms，fill both | 默认收敛为较轻的 0.035em；求解 CSS cubic-bezier 的 x，再算 y，达到终点后保持 |
...
| 附加上浮 | AMLL 强调字符提前 400ms，持续时间 ×1.4，并允许动画叠加 | 当前版本移除附加正弦位移，只保留轻微普通上浮，避免浮起后回落 |

## 本轮预览优化

浮起状态允许相邻字词的普通上浮动画重叠。每个单元从播放开始以 ease-out 轻微上移，达到 0.035em 后保持，不再使用会回落的附加正弦位移。逐字错落仍只影响缩放、位移和光晕，不改变这条稳定的基础上浮轨迹。

时间表达式会在建立分词范围时缓存 UTF-8 长度、去空白长度和中文判断，单元循环不再重复创建字符数组、裁剪字符串或扫描中文码点。
